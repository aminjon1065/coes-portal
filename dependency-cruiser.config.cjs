  ...
  forbidden: [
    ...
    },
    {
      ...
      comment:
        ...
        'То же и с tools/audit-verify: docs/05-ДОСТУП.md § 9.3 требует команды ' +
        'pnpm audit:verify, которая проходит цепочку журнала в базе. ' +
        'Противоречие между документами разрешено старшинством (00-КОНТРАКТ и ' +
        '05-ДОСТУП старше 03-АРХИТЕКТУРА в перечне § 3) и записано как В-12. ' +
        'Исключения перечислены поимённо, каталог tools целиком НЕ открыт.',
      from: {
        pathNot: '^(packages/db|apps/api|apps/worker|tools/check-urls|tools/audit-verify)/',
      },
      to: { path: '^packages/db/' },
    },
  ],
